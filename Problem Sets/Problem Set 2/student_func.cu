    ...
    int c = thread_2D_pos.x;
      ...
      for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r) {
        for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; ++filter_c) {
          ...
          float image_value = static_cast<float>(inputChannel[thread_1D_pos]);
      ...
      outputChannel[thread_1D_pos] = static_cast<int>(result);    
                        ...
                        const int filterWidth)
{
  ...
  const dim3 blockSize = dim3(filterWidth, filterWidth, 1);

  //TODO:
  //Compute correct grid size (i.e., number of blocks per kernel launch)
  //from the image size and and block size.
  const dim3 gridSize = dim3(numCols/filterWidth + 1, numRows/filterWidth + 1, 1);
